...
void parallel_bucket_sort(vector<double>& data, int n, int rank, int size,double& total_parallel_time, double& master_serial_time) {
    ...
    if (rank == 0) {
        ...
        end_time = MPI_Wtime();
        master_serial_time += end_time - start_time;  // Record master serial setup time
    }
    ...
    if (rank == 0) {
        ...
        cout << "Master process serial work time: " << master_serial_time << " seconds" << endl;
    }
}
...
int main(int argc, char* argv[]) {
    ...
    double master_serial_time = 0.0; //Defining Serial Time
    ...
    if (rank == 0) {
     ...
     parallel_bucket_sort(data, n, rank, size, total_parallel_time,master_serial_time);
    
    //Calculating the speedup of Gustafson’s Law
    double f = master_serial_time / total_parallel_time;
    double predicted_speedup = size + f * (1 - size);
    cout << "Predicted Speedup (Gustafson's Law): " << predicted_speedup << endl;
