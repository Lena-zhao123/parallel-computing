...
int main(int argc, char* argv[]) {
    ...
    // Print task_id and num_tasks for debugging
    printf("Process %d of %d: task_id = %d, num_tasks = %d\n", task_id, num_tasks, task_id, num_tasks);
    fflush(stdout);
    ...
    if (task_id == MASTER) {
        ...
        // Communication time for the Master process to send seed and constants
        s_t2 = MPI_Wtime();
        ...
        // Receive the seed, a, and c sent by the master process
    ...
    printf("Process %d: Starting round-robin computation with total points %llu.\n", task_id, total_num_points);
    fflush(stdout);

    // Each process handles points in a round-robin fashion
